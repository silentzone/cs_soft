/**
* 防WebUboss整合苹果系统导航菜单界面 you.guess
* @2013.04.06 
*/
...
$(function() {
	...
	Navbar.init();//初始化导航条 取消顶部导航 you.guess
	Arrows.init();//初始化分页箭头
...
Deskpanel = function(me){
	...
	return me ={
		init:function(ops){
			...
			me.refresh();
			me.bindEvent(); //暂时去掉拖拽效果 you.guess
		...
		create:function(){
			...
			me.box.css({"left": "45px","right": "0px"});
...
// 			// me.addToolList();
// 			// me.initDrag();
// 			me.createThemeTool();// 设置主题信息提前，左边栏去除 you.guess
...
// 			//me.start = $("<input title='点击这里开始' type='button' class='dock_tool_icon dock_tool_start'>");
...
// 		menu+="<li><a href=\"#\" id=\"desk_"+(i+1)+"\" name=\"desk_"+(i+1)+"\">"+
// 			"<img src=\""+data[i].icon+"\" /><span>"+data[i].name+"</span></a></li>";
// 	}
// 	// var _box = "<div id='dock' class='dock' ><div class='dock-container'>"+ menu +"</div></div>";
// 		var _box = "<div id='dock_bottom'><ul>"+menu+"</ul></div>";
// 	return me = {
// 		init :function(){
// 			me.create();			
...
// 	var mome ="<div id=\"mome\"><div class=\"infotitl\"><iframe width=\"420\" scrolling=\"no\" height=\"60\" frameborder=\"0\" allowtransparency=\"true\" src=\"http://www.tianqi.com/index.php?c=code&id=12&icon=1&num=5\"></iframe></div></div>";
// 	var inform="<div class='panel_info' > <h3> 标题通知 </h3> <p>Uboss虚拟桌面通知<br><br><br><br><br></p> </div>";
// 	var _msgBox = "<div id=\"msg\"><div id=\"msg_close\"><a href=\"#\">关闭</a></div>&nbsp;&nbsp;Uboss虚拟桌面消息</div>";
// 	return ep = {
// 		init:function(){
// 			ep.create();
// 			ep.bindEvent();
...
// 					arrows.changeStyle(t,thisPage); //取消顶部导航 you.guess
...
// 			var ah = 112/2;
// 			$("#arrows_l").css({"top":((wh/2)-ah)+"px","left":"30px"});
// 			$("#arrows_r").css({"top":((wh/2)-ah)+"px","right":"30px"});
// 		},
			...
			if(taskItem.length==1){
				...
				if(jsonSonMenu.length>0){
					...
					var _box ="<div id=\"sonMenu_box"+id+"\" class=\"sonMenuListBox\"><div class=\"sonMenu_line\">"+
						"<span class=\"sonMenu_font\">可订购服务</span></div>"+sonlist+"</div>";
					...
					art.dialog.open(url,/** 弹出ART窗体*/
						{   
							"id" :id,
							title: title,	
							width:width,//980 设置窗口宽度自动适应width
							height:height,
							close:function(){
								me.closeMinTask(id);
								$("#sonfile_a"+id).remove();
...
// 			 var title = $.trim(_this.text());
// 			 var url =_this.attr("url");
// 			 var type = _this.attr("apptype");
// 			 if(type=='0'){// 大栏目
// 				 // 类同顶上栏目，打开potal容器不打开页面 暂时用url 来存储容器编号 you.guess 
// 				var t = url,c = thisPage;
// 				if(t==c){//目标页数与当前页数相同时返回
// 					return;
// 				}
// 				var left = 0;
...
// 显示全局 ICON LIST
allAppPanel = function() {
	 	//  must搞清楚几个个概念 
	 	//1 bindEvent 中 this  如何实现 init me.bindEvent(); 的效果 
	 	//2 实现对象的方法 
	 	//Desktop=function(me){  return me={}; }();  方法后直接运行创建 
	 	// for 循环中 中断 跳出 中断顶层 循环
	 	// 将ALLAPP 方法 抽象重构一下 	

	 	// 需要加上判断防止打开多个全局APPlist
	 	...
	 	return me = { 
			...
			create:function () {
				var opsIdx =function(str){// 返回ICON 索引 
				    return str.replace("Icon",""); 	
				}
				for( var a in ops ) {
					var idx = opsIdx(a) - 1;
					var $col = $("<div id='" + a + "' class='app_col'/>");
					var $top = $("<div class='col_top'></div>")
					var name = DATA.menu[idx].name;
					$col.append(name);

			 	    for(var i=0; i< Deskpanel.Icon.length; i++) { 
				 		var o = Deskpanel.Icon[i].app;
				 		 
				 		 //
			 	     	 for( var j=0; j<ops[a].length; j++ ) {
			 	     	 	if(ops[a][j]== Deskpanel.Icon[i].fid ) {
			 	     	 		// alert(a +"   ===   " + ops[a][j]);
			 	     	 		var $a =  $("<div class='app_icon' />");
						 		var $img = $("<img   src='icon/" + o.icon +"' />")
						 		$a.attr({
						 			id:o.appid + "_all",
						 			title:o.name,
						 			url:o.url,
