...
function loadData(){
	...
	for ( i =0;i<templist.getDataObjCnt("TEMPLETITEM");i++){
		...
		createTabGroups(tempItem,tab_id,2);
	}
...
function getAllControlValue(dict){
    ...
    if (tags != null && tags.length > 0) {
        for (var i = 0; i < tags.length; i++) {  
			...
			if (controlid =="orders"){
				var vm_orders = tags[i].value;
				if (vm_orders == null || vm_orders.length ==0 ){
					$.messager.alert('温馨提醒',"请输入数量",'info');
					return 0;
				}
				//判断是否为数字
				if(isNaN(vm_orders)){
				   $.messager.alert('温馨提醒',"数量必须为数字",'info');
					return 0;
				}
				dict.setValue("ORDERS",tags[i].value); //备注
			}
...
function openThdSaveWin(obj){
	...
	//得到费用总额
	var dict = new DynamicDict("UBOSS_USERMGR_001");
	if (getAllControlValue(dict) ==0 ) return ;
