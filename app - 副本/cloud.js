...
function loadData(){
	...
	for ( i =0;i<templist.getDataObjCnt("TEMPLETITEM");i++){
		...
		if (left(tab_id,8) =='USERINST' && judge==0){
			$("#tablist").append('<li class="nav-header">已购买的模板</li>');
			judge =1;
		}
		//插入tablist
		var tab_name_2 =tab_name;
		if (judge ==1){
			if (state =='1') tab_name_2='<已审>' + tab_name;
			if (state =='0') tab_name_2='<新制>' + tab_name;
		}
		if( tab_id == 1) {
			$("#tablist").append(' <li class="active"><a href="#WH'+tab_id+'"><i class="icon-align-justify"></i> '+tab_name_2+' </a></li> ');
		} else {
			$("#tablist").append(' <li><a href="#WH'+tab_id+'"><i class="icon-align-justify"></i> '+tab_name_2+' </a></li> ');
		}
...
function creatButton(mydiv){
	...
	var cpuControlDiv = document.createElement("div");
	cpuControlDiv.setAttribute("class","control-group");
	
	
	cpuControlDiv.innerHTML =' <button type="submit" class="btn btn-primary" style="margin-right:5px;" onClick="saveMGE(this);">保存</button><button type="button" class="btn">重置</button> ';
	var wrapDiv =  document.createElement("div");
	wrapDiv.setAttribute("class","controls"); 
	wrapDiv.appendChild(cpuControlDiv);
	mydiv.appendChild(wrapDiv);
